...
directly, encrypted HTTP submission, and independent recovery. Its contracts, SDK flows, and local
...
- **Independent recovery:** a separate recovery implementation and standalone CLI support public
  asset/position exits and delegation revocation, including when the main implementation is unavailable.
- **Core SDKs:** shared models, Engine, and Wallet expose account allocation, proof preparation,
  arbitrary calls, submission, and recovery without requiring protocol-specific recipes.
...
Web / Desktop
  ...
  -> Wallet SDK          accounts, sync, balances, encrypted operation journals
       ...
       -> Recovery SDK   shared recovery-plan validation
...
Standalone recovery CLI
  -> Execution EOA delegated to MatrixRecovery7702
  -> Public exits and sweeps, then delegation revocation
...
remain local. Recovery can run independently of the normal Wallet and HTTP operator.

EIP-7702 requires chain and RPC support. Arbitrary calls do not guarantee compatibility with every
protocol: caller-code checks, signatures, and recovery paths require validation. DeFi positions and
...
| [wallet](https://github.com/matrix-privacy/wallet) | Account lifecycle, transaction journals, HTTP submission and recovery | Alpha package published; 7702 source candidate |
| [contracts](https://github.com/matrix-privacy/contracts) | Privacy pool plus account, recovery and probe implementations | Pool deployed on Sepolia; 7702 local candidate |
...
| `matrix-account-recovery` | Independent recovery library and standalone CLI | Local candidate; GitHub remote pending |
...
- Finish independent recovery and full security audits before enabling public 7702 deployments.
- Validate operator retention, key rotation, and longer pending/replacement/reorg runs.
- Publish coordinated SDK versions, reviewed deployment pins, and signed recovery manifests.
...
新的 **MatrixAccount7702 候选实现**增加了直接调用 DeFi 的执行 EOA、加密 HTTP 提交和独立
恢复能力。合约、SDK 流程与本地集成测试已实现，经过审计的公共部署和产品发布仍待完成。
...
- **独立恢复：** 独立 recovery implementation 与单文件 CLI 支持公开退出头寸、转出资产和撤销
  delegation，包括主实现不可用时的恢复。
- **核心 SDK：** shared models、Engine、Wallet 提供账户分配、proof 准备、任意合约调用、提交和
  恢复能力，无需为每个协议新增 recipe。
...
独立 recovery CLI
  -> 委托给 MatrixRecovery7702 的执行 EOA
  -> 公开退出与资产转出，再撤销 delegation
...
公共数据服务提供可验证的公开链上数据；钱包密钥、note 解密和 proving 留在本地。独立恢复
可以脱离常规 Wallet 与 HTTP operator 运行。

EIP-7702 需要底层链与 RPC 支持。任意调用能力不保证每个协议都兼容：caller code 检查、签名和
恢复路径都需要验证。DeFi 头寸与执行账户活动是公开的，重新 Shield 不会抹除这些历史，复用
...
| [wallet](https://github.com/matrix-privacy/wallet) | 账户生命周期、交易 journal、HTTP 提交与恢复 | Alpha 包已发布；7702 源码候选版 |
| [contracts](https://github.com/matrix-privacy/contracts) | 隐私池与 account、recovery、probe 实现 | 隐私池已部署 Sepolia；7702 本地候选版 |
...
| `matrix-account-recovery` | 独立恢复库与单文件 CLI | 本地候选版；GitHub 远端待配置 |
...
- 启用公共 7702 部署前完成独立恢复审计与完整安全审计。
- 验证 operator 数据保留、密钥轮换和更长时间的 pending/replacement/reorg 场景。
- 发布协调后的 SDK 版本、已审核 deployment pins 和签名 recovery manifest。
